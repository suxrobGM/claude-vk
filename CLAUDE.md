...
Policies: only DMs have one (`policy` = `pairing` (default) or `allowlist`). Group chats are off by default and opt in by `peer_id` via `POST /access/groups` (`/vk:access group add`); they have no pairing flow. Full surface in [ACCESS.md](ACCESS.md).
